...
- Updated the all-metric spectral-biodiversity scatterplot workflow to include all requested PCA metrics: raw and standardized alpha hull, Rao's Q, mean distance, convex hull, 3D hull volume, and 3D hull area, with spectral angle entropy retained for comparison. Regenerated outputs under `reports/analysis/20260819_spectral_biodiversity_all_metric_scatter_analysis.md`, `reports/tables/spectral_biodiversity_all_metrics/`, and `reports/figures/spectral_biodiversity_all_metrics/`; the current suite contains 13 original figures and 273 untransformed spectral-biodiversity relationship rows. Added two power-2 transformation figure families in the same layout: `02_power2_biodiversity_original_spectral_*` squares biodiversity metrics only, and `03_power2_biodiversity_power2_spectral_*` squares both biodiversity and spectral metrics; their 546 relationship summaries are stored in `spectral_biodiversity_power2_transform_relationships.csv`. Added `reports/figures/spectral_biodiversity_phylogenetic_transform_stacks/` with 13 tall phylogenetic-only stack figures, one per spectral metric, where each phylogenetic metric is shown as regular, both-axis power-2, and biodiversity-only power-2 rows across the 10 m, 20 m, and 50 m scale columns. Added `04_std_pca_volume_metrics_vs_all_biodiversity_metrics_by_scale.png` as a single normal-value summary comparing standardized PCA alpha hull, convex hull, 3D hull volume, and 3D hull area against all biodiversity metrics. The strongest absolute untransformed relationships remain concentrated at 50 m and involve phylogenetic metrics, led by standardized PCA alpha hull with abundance-weighted Faith's PD (r = 0.497, R2 = 0.247), standardized PCA alpha hull with phylogenetic Rao's Q (r = 0.487, R2 = 0.238), and standardized PCA mean distance with phylogenetic Rao's Q (r = 0.444, R2 = 0.197).
- Decision update, 2026-08-20: convex hull and 3D hull volume/area metrics will not be carried forward in future analyses or the main manuscript result set. Existing convex/3D hull tables and figures remain archived diagnostic outputs from metric screening. Future PCA-centered work should prioritize standardized PCA alpha hull, standardized PCA mean distance, and selected Rao-style metrics, with raw PCA counterparts retained only as brightness-sensitive comparisons.
...
- Because convex hull and 3D hull metrics are no longer forward analysis metrics, their method notes should be treated as archive/provenance details rather than manuscript-priority methods.
