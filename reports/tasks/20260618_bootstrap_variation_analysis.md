...
- The bootstrap means are usable as primary spectral heterogeneity means when quality-control fields are carried forward.
...
- Some quadrats show elevated bootstrap CV and should be flagged:
  - 10 m: 372 of 1,866 bootstrap-mean quadrats exceed 5% CV; 127 exceed 10% CV.
  - 20 m: 60 of 477 exceed 5% CV; 12 exceed 10% CV.
  - 50 m: 11 of 79 exceed 5% CV; 0 exceed 10% CV.
- The 95th percentile bootstrap CV across all bootstrap-mean quadrats is 10.4%.
...
Use `spectral_entropy` / `boot_mean` as the main heterogeneity mean, but carry `boot_sd`, `boot_cv`, bootstrap-mean CI fields, and `method` into downstream tables. Do not document bootstrap-mean rows as exhaustive all-pixel pairwise calculations; only rows with `method = "exact_all_pixels"` used all retained pixel pairs. Run sensitivity checks that flag or exclude high-CV or wide-CI quadrats.
