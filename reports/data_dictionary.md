...
- Description: Derived analysis tables generated by `scripts/3_Analysis/spectral_biodiversity_all_metric_scatter_analysis.R`. These outputs compare all seven focal spectral heterogeneity measures against all seven biodiversity metrics across 10 m, 20 m, and 50 m scales using all complete quadrat records for each pair, with no edge filtering.
- Source Location: `reports/tables/spectral_biodiversity_all_metrics/`
- Row Count: 2,580 rows in `spectral_biodiversity_all_metric_dataset.csv`; 147 rows in `spectral_biodiversity_all_metric_relationships.csv`
- Column Count: 16 columns in the dataset; 12 columns in the relationship summary
...
- Date Fields: none detected
- Numeric Fields: seven spectral heterogeneity measures, seven biodiversity metrics, `n`, `pearson_r`, `r_squared`, `f_statistic`, `f_p_value`, `slope`, `intercept`
- Categorical Fields: `scale`, `quad_id`, metric identifiers, and display labels
...
| File | Description |
|---|---|
| `spectral_biodiversity_all_metric_dataset.csv` | Combined all-scale dataset containing quadrat ID, scale, seven spectral heterogeneity metrics, and seven biodiversity metrics |
...
- Description: Derived analysis tables generated by `scripts/3_Analysis/spectral_heterogeneity_relationship_analysis.R`. These outputs compare all seven focal spectral heterogeneity measures in one integrated pairwise suite: spectral angle entropy, raw PCA alpha-hull area, raw PCA spectral Rao's Q, raw PCA mean Euclidean distance, standardized PCA alpha-hull area, standardized PCA spectral Rao's Q, and standardized PCA mean Euclidean distance across 10 m, 20 m, and 50 m quadrats. Companion tables quantify pairwise correlations, elevation-adjusted relationships, metric coverage, species-composition separation in spectral metric space, quadrat-level pixel brightness for 563 nm plus blue, green, red, and near-infrared spectral regions, and direct spectral-metric relationships with regional illumination.
- Source Location: `reports/tables/spectral_heterogeneity_relationships/`
- Row Count: 63 rows in `spectral_metric_pairwise_correlations.csv`; 63 rows in `spectral_metric_elevation_adjusted_models.csv`; 21 rows in `spectral_metric_summary.csv`; 315 rows in `spectral_composition_scatterplot_silhouette.csv`; 2,474 rows in `quadrat_pixel_brightness_summary.csv`; 84 rows in `spectral_metric_regional_illumination_correlations.csv`
- Column Count: varies by output table
