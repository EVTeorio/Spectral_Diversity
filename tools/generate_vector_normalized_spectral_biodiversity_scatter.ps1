...
function Get-StatRow($rows, $scale, $metric) {
  return @($rows | Where-Object {
    $_.scenario -eq "identity" -and
    $_.scale -eq $scale -and
    $_.spectral_metric -eq $metric -and
    $_.biodiversity_metric -eq "phy_afaith"
  })[0]
}
...
function Draw-Panel($g, $panel) {
  ...
  $xRange = Nice-Range @($rows | ForEach-Object { $_.phy_afaith })
  $yRange = Nice-Range @($rows | ForEach-Object { $_.$metric })
  $xmin = $xRange[0]
  $xmax = $xRange[1]
  $ymin = $yRange[0]
  $ymax = $yRange[1]
  ...
  foreach ($row in $rows) {
    $px = Scale-X ([double]$row.phy_afaith) $xmin $xmax $left $right
    $py = Scale-Y ([double]$row.$metric) $ymin $ymax $top $bottom
    $g.FillEllipse($pointBrush, [single]($px - 2.5), [single]($py - 2.5), 5.0, 5.0)
  }
  ...
  for ($i = 0; $i -le 4; $i++) {
    ...
    Draw-Centered $g ("{0:0}" -f $xVal) ($xPos - 42) ($bottom + 7) 84 24 14 $muted
    Draw-Centered $g ("{0:0.0}" -f $yVal) ($left - 82) ($yPos - 12) 70 24 14 $muted
  }

  $annotation = (Format-R $stat.pearson_r) + "; " + (Format-P $stat.f_p_value)
  Draw-Text $g $annotation ($left + 13) ($top + 12) 17 $ink ([System.Drawing.FontStyle]::Bold)
...
$scales = @("10m", "20m", "50m")
$metrics = @(
  @{ Name = "spec_spca_mean"; Label = "Mean distance" },
  @{ Name = "spec_spca_rao"; Label = "Spectral Rao's Q" },
  @{ Name = "spec_spca_alpha"; Label = "Alpha-hull area" }
)
...
Draw-Text $g "Vector-normalized spectral heterogeneity and abundance-weighted Faith's PD" 150 75 40 $ink ([System.Drawing.FontStyle]::Bold)
Draw-Text $g "Identity-scale correlations across quadrat grain" 150 132 24 $muted
...
for ($c = 0; $c -lt $scales.Count; $c++) {
  Draw-Centered $g ($scales[$c] + " quadrats") $lefts[$c] 235 $panelW 45 28 $ink ([System.Drawing.FontStyle]::Bold)
  $scaleN = (Get-StatRow $relationships $scales[$c] "spec_spca_mean").n
  Draw-Centered $g ("n = " + $scaleN) $lefts[$c] 275 $panelW 32 20 $muted
}

for ($r = 0; $r -lt $metrics.Count; $r++) {
  Draw-Text $g $metrics[$r].Label 400 ($tops[$r] - 38) 24 $ink ([System.Drawing.FontStyle]::Bold)
}

for ($r = 0; $r -lt $metrics.Count; $r++) {
  for ($c = 0; $c -lt $scales.Count; $c++) {
    $scale = $scales[$c]
    $metric = $metrics[$r].Name
    $rows = @($dataset | Where-Object {
      $_.scale -eq $scale -and
      $_.phy_afaith -ne "" -and $_.phy_afaith -ne "NA" -and
...
Draw-Centered $g "Abundance-weighted Faith's phylogenetic diversity" 650 2055 1600 45 27 $ink ([System.Drawing.FontStyle]::Bold)
...
$g.DrawString("Vector-normalized PCA spectral heterogeneity", $font, $brush, [System.Drawing.RectangleF]::new(45, 700, 54, 950), $format)
$format.Dispose()
$font.Dispose()
$brush.Dispose()

$outPath = Join-Path $outDir "10_vector_normalized_spectral_biodiversity_abundance_weighted_faith_by_scale.png"
