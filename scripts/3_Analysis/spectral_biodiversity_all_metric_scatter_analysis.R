...
STACK_FIGURE_DIR <- file.path(PROJECT_DIR, "reports/figures/spectral_biodiversity_phylogenetic_transform_stacks")
...
PHYLOGENETIC_METRICS <- c("phy_faith", "phy_rao", "phy_afaith")
STD_PCA_VOLUME_METRICS <- c("spec_spca_alpha", "spec_spca_convex", "spec_spca_hull3d_v", "spec_spca_hull3d_a")
...
dir.create(STACK_FIGURE_DIR, recursive = TRUE, showWarnings = FALSE)
...
power2_values <- function(values, apply_power2 = FALSE) {
  if (apply_power2) {
    values^2
  } else {
    values
...
axis_label <- function(metric, apply_power2 = FALSE) {
...
relationship_row <- function(data, scale_name, spectral_metric, biodiversity_metric, scenario = "identity", spectral_power2 = FALSE, biodiversity_power2 = FALSE) {
  ...
  row <- data.frame(
    ...
    spectral_transform = ifelse(spectral_power2, "power_2", "identity"),
    biodiversity_metric = biodiversity_metric,
    biodiversity_label = display_name(biodiversity_metric),
    biodiversity_transform = ifelse(biodiversity_power2, "power_2", "identity"),
    n = sum(complete),
    pearson_r = NA_real_,
    r_squared = NA_real_,
  ...
  if (sum(complete) >= 3) {
    model_data <- data.frame(
      x = power2_values(scale_data[[biodiversity_metric]][complete], biodiversity_power2),
      y = power2_values(scale_data[[spectral_metric]][complete], spectral_power2)
    )
...
summarize_power2_relationships <- function(data) {
  scenarios <- list(
    biodiversity_power2_spectral_identity = list(spectral_power2 = FALSE, biodiversity_power2 = TRUE),
    biodiversity_power2_spectral_power2 = list(spectral_power2 = TRUE, biodiversity_power2 = TRUE)
  )
  rows <- list()
  index <- 1
  for (scenario in names(scenarios)) {
    ...
    for (spectral_metric in SPECTRAL_METRICS) {
      for (biodiversity_metric in BIODIVERSITY_METRICS) {
        for (scale_name in SCALES) {
          rows[[index]] <- relationship_row(
            data,
            scale_name,
            spectral_metric,
            biodiversity_metric,
            scenario = scenario,
            spectral_power2 = settings$spectral_power2,
            biodiversity_power2 = settings$biodiversity_power2
          )
          index <- index + 1
        }
      }
...
plot_panel <- function(data, relationships, scale_name, spectral_metric, biodiversity_metric, spectral_power2 = FALSE, biodiversity_power2 = FALSE) {
  scale_data <- data[data$scale == scale_name, , drop = FALSE]
  complete <- !is.na(scale_data[[spectral_metric]]) & !is.na(scale_data[[biodiversity_metric]])
  point_col <- if (grepl("^phy_", biodiversity_metric)) "#315C8A" else "#3D7B45"
  plot(
    power2_values(scale_data[[biodiversity_metric]], biodiversity_power2),
    power2_values(scale_data[[spectral_metric]], spectral_power2),
    pch = 16,
    cex = 0.50,
    col = adjustcolor(point_col, alpha.f = 0.58),
    xlab = axis_label(biodiversity_metric, biodiversity_power2),
    ylab = axis_label(spectral_metric, spectral_power2),
  ...
  if (nrow(row) == 1 && !is.na(row$pearson_r) && sum(complete) >= 3) {
    fit <- lm(
      power2_values(scale_data[[spectral_metric]][complete], spectral_power2) ~
        power2_values(scale_data[[biodiversity_metric]][complete], biodiversity_power2)
    )
...
save_spectral_metric_figures <- function(data, relationships, file_prefix = "01", title_suffix = "", spectral_power2 = FALSE, biodiversity_power2 = FALSE) {
  files <- character(length(SPECTRAL_METRICS))
  for (i in seq_along(SPECTRAL_METRICS)) {
    spectral_metric <- SPECTRAL_METRICS[i]
    file_path <- file.path(
      FIGURE_DIR,
      paste0(file_prefix, "_", SHORT_NAMES[[spectral_metric]], "_vs_all_biodiversity_metrics_by_scale.png")
    )
    ...
    for (biodiversity_metric in BIODIVERSITY_METRICS) {
      for (scale_name in SCALES) {
        plot_panel(data, relationships, scale_name, spectral_metric, biodiversity_metric, spectral_power2, biodiversity_power2)
      }
    }
    mtext(
      paste0(axis_label(spectral_metric, spectral_power2), " vs biodiversity metrics across scales", title_suffix),
      outer = TRUE,
      cex = 1.2,
      font = 2
    )
...
save_power2_figures <- function(data, transformed_relationships) {
  c(
    save_spectral_metric_figures(
      data,
      transformed_relationships[transformed_relationships$scenario == "biodiversity_power2_spectral_identity", ],
      file_prefix = "02_power2_biodiversity_original_spectral",
      title_suffix = " (biodiversity metrics squared)",
      spectral_power2 = FALSE,
      biodiversity_power2 = TRUE
    ),
    save_spectral_metric_figures(
      data,
      transformed_relationships[transformed_relationships$scenario == "biodiversity_power2_spectral_power2", ],
      file_prefix = "03_power2_biodiversity_power2_spectral",
      title_suffix = " (biodiversity and spectral metrics squared)",
      spectral_power2 = TRUE,
      biodiversity_power2 = TRUE
    )
  )
}

phylogenetic_stack_scenarios <- function() {
  data.frame(
    scenario = c(
      "identity",
      "biodiversity_power2_spectral_power2",
      "biodiversity_power2_spectral_identity"
    ),
    label = c(
      "Regular",
      "Power-2 both axes",
      "Power-2 biodiversity only"
    ),
    spectral_power2 = c(FALSE, TRUE, FALSE),
    biodiversity_power2 = c(FALSE, TRUE, TRUE),
    stringsAsFactors = FALSE
  )
}

plot_stack_panel <- function(data, relationships, scale_name, spectral_metric, biodiversity_metric, scenario_row) {
  scale_data <- data[data$scale == scale_name, , drop = FALSE]
  complete <- !is.na(scale_data[[spectral_metric]]) & !is.na(scale_data[[biodiversity_metric]])
  plot(
    power2_values(scale_data[[biodiversity_metric]], scenario_row$biodiversity_power2),
    power2_values(scale_data[[spectral_metric]], scenario_row$spectral_power2),
    pch = 16,
    cex = 0.42,
    col = adjustcolor("#315C8A", alpha.f = 0.56),
    xlab = axis_label(biodiversity_metric, scenario_row$biodiversity_power2),
    ylab = axis_label(spectral_metric, scenario_row$spectral_power2),
    main = paste0(SCALE_LABELS[[scale_name]], ": ", scenario_row$label)
  )

  row <- relationships[
    relationships$scenario == scenario_row$scenario &
      relationships$scale == scale_name &
      relationships$spectral_metric == spectral_metric &
      relationships$biodiversity_metric == biodiversity_metric,
  ]
  if (nrow(row) == 1 && !is.na(row$pearson_r) && sum(complete) >= 3) {
    fit <- lm(
      power2_values(scale_data[[spectral_metric]][complete], scenario_row$spectral_power2) ~
        power2_values(scale_data[[biodiversity_metric]][complete], scenario_row$biodiversity_power2)
    )
    abline(fit, col = "#111111", lwd = 2)
    legend(
      "topleft",
...
save_phylogenetic_transform_stack_figures <- function(data, relationships, transformed_relationships) {
  scenarios <- phylogenetic_stack_scenarios()
  all_relationships <- rbind(relationships, transformed_relationships)
  files <- character(length(SPECTRAL_METRICS))
  for (i in seq_along(SPECTRAL_METRICS)) {
    spectral_metric <- SPECTRAL_METRICS[i]
    file_path <- file.path(
      STACK_FIGURE_DIR,
      paste0("01_phylogenetic_transform_stack_", SHORT_NAMES[[spectral_metric]], ".png")
    )
    png(file_path, width = 3900, height = 8600, res = 300)
    old_par <- par(no.readonly = TRUE)
    par(mfrow = c(length(PHYLOGENETIC_METRICS) * nrow(scenarios), length(SCALES)), mar = c(3.8, 4.5, 2.2, 0.8), oma = c(0, 0, 3.0, 0))
    for (biodiversity_metric in PHYLOGENETIC_METRICS) {
      for (scenario_index in seq_len(nrow(scenarios))) {
        scenario_row <- scenarios[scenario_index, ]
        for (scale_name in SCALES) {
          plot_stack_panel(data, all_relationships, scale_name, spectral_metric, biodiversity_metric, scenario_row)
        }
      }
    }
    mtext(
      paste0(display_name(spectral_metric), ": phylogenetic biodiversity transform stack"),
      outer = TRUE,
      cex = 1.15,
      font = 2
    )
...
save_std_pca_volume_metric_biodiversity_figure <- function(data, relationships) {
  file_path <- file.path(
    FIGURE_DIR,
    "04_std_pca_volume_metrics_vs_all_biodiversity_metrics_by_scale.png"
  )
  png(file_path, width = 4200, height = 22000, res = 300)
  old_par <- par(no.readonly = TRUE)
  par(mfrow = c(length(STD_PCA_VOLUME_METRICS) * length(BIODIVERSITY_METRICS), length(SCALES)), mar = c(3.8, 4.6, 2.1, 0.8), oma = c(0, 0, 3.0, 0))
  for (spectral_metric in STD_PCA_VOLUME_METRICS) {
    for (biodiversity_metric in BIODIVERSITY_METRICS) {
      for (scale_name in SCALES) {
  ...
  mtext(
    "Standardized PCA hull and volume metrics vs biodiversity metrics across scales",
    outer = TRUE,
    cex = 1.15,
    font = 2
  )
...
write_reports <- function(data, relationships, figure_files, transformed_relationships, transformed_figure_files, stack_figure_files, std_volume_figure_file) {
  ...
  analysis_lines <- c(
    ...
    "",
    paste0("- `", relative_path(transformed_figure_files), "`"),
    "",
    "## Phylogenetic Transformation Stack Figures",
    "",
    paste0("- `", relative_path(stack_figure_files), "`"),
    "",
    "## Standardized PCA Hull And Volume Summary Figure",
    "",
    paste0("- `", relative_path(std_volume_figure_file), "`"),
    "",
    "## Tables",
    "",
  ...
  validation_lines <- c(
    ...
    paste0("- Power-2 transformation relationship rows: ", nrow(transformed_relationships)),
    paste0("- Expected power-2 transformation relationship rows: ", 2 * length(SPECTRAL_METRICS) * length(BIODIVERSITY_METRICS) * length(SCALES)),
    paste0("- Missing Pearson r values: ", sum(is.na(relationships$pearson_r))),
    paste0("- Missing power-2 transformation Pearson r values: ", sum(is.na(transformed_relationships$pearson_r))),
    paste0("- Output figures present: ", sum(file.exists(figure_files)), " of ", length(figure_files)),
    paste0("- Power-2 transformation figures present: ", sum(file.exists(transformed_figure_files)), " of ", length(transformed_figure_files)),
    paste0("- Phylogenetic transformation stack figures present: ", sum(file.exists(stack_figure_files)), " of ", length(stack_figure_files)),
    paste0("- Standardized PCA hull and volume summary figure present: ", sum(file.exists(std_volume_figure_file)), " of 1"),
    paste0("- Output tables present: ", sum(file.exists(file.path(TABLE_DIR, c(
      ...
      "spectral_biodiversity_power2_transform_relationships.csv"
    )))), " of 3"),
    "",
    "## Result",
    "",
...
run_spectral_biodiversity_all_metric_scatter_analysis <- function() {
  ...
  transformed_relationships <- summarize_power2_relationships(data)
  figure_files <- save_spectral_metric_figures(data, relationships)
  transformed_figure_files <- save_power2_figures(data, transformed_relationships)
  stack_figure_files <- save_phylogenetic_transform_stack_figures(data, relationships, transformed_relationships)
  std_volume_figure_file <- save_std_pca_volume_metric_biodiversity_figure(data, relationships)
  ...
  write.csv(transformed_relationships, file.path(TABLE_DIR, "spectral_biodiversity_power2_transform_relationships.csv"), row.names = FALSE)
  write_reports(data, relationships, figure_files, transformed_relationships, transformed_figure_files, stack_figure_files, std_volume_figure_file)
  ...
  invisible(list(
    ...
    transformed_relationships = transformed_relationships,
    figure_files = figure_files,
    transformed_figure_files = transformed_figure_files,
    stack_figure_files = stack_figure_files,
    std_volume_figure_file = std_volume_figure_file
  ))
}
