...
output_folder    <- "Spectral_Diversity/Indices_SHPs"
crs_proj         <- 26916
window_radius_m  <- 20
...
pd_vec <- vapply(
  ...
  function(i) {
    ...
    tips_present <- unique(tip_vec[i])
    tips_present <- tips_present[tips_present %in% all_tips]
    
    if (length(tips_present) == 0) return(0)
    if (length(tips_present) == 1) return(1)
    
    sub_tree <- ape::keep.tip(phylo_tree, tips_present)
...
beep()
# GRID
col_name <- paste0("faithPD_", window_radius_m, "m")
...
out_raster <- file.path(output_folder,
                        paste0("faithPD_", window_radius_m, "m.tif"))
