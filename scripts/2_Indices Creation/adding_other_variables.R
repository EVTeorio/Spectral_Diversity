...
csv_data <- read.csv("Quad_Values/20m_spectral_sp.csv")
  #dplyr::select(-X)

quads_with_elevation <- st_read("Quad_Values/Other_variables/20m_elevation.shp")
...
write.csv(merged_data, "Quad_Values/20m_spectral_sp.csv", row.names = FALSE) 
head(merged_data)
